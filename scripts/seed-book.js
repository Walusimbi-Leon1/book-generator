...
// ── Clone template ─────────────────────────────────────────────────
console.log(`Cloning template: sgss-template/book-binder`);
const tmpDir = `/tmp/seed-${REPO}-${Date.now()}`;
execSync(`git clone https://x-access-token:${GH_TOKEN}@github.com/sgss-template/book-binder.git ${tmpDir}`, {
  stdio: "inherit",
});
